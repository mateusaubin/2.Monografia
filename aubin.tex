...
% key capabilities: get, set, trap stallings p78

% The operations available in SNMP are quite limited, and yet quite powerful
% p 169 c6 \cite{perkins1997understanding}

% Having just a few simple operations has allowed the implementationsof the
% protocol engines to be quite small and require fewer resources to develop, 
% as compared with other management protocols.
% p 185 c6 \cite{perkins1997understanding}

% dada a estrutura da PDU (vista a seguir) todas as operações podem ser
% efetuadas sobre mais de um elemento
...
\begin{table}
	...
	\begin{minipage}{.75\textwidth}
		\begin{tabular*}{.90\textwidth}{|l|l|l|l|}
	\hline
		\textbf{Operação}   &  
		\textbf{De -- Para} & 
		\textbf{Requisição} & 
		\textbf{Resposta}   \\ 
	\hline
		GET     & Gerente -- Agente   & get-request      & response \\
		GETNEXT & Gerente -- Agente   & get-next-request & response \\
		GETBULK & Gerente -- Agente   & get-bulk-request & response \\
		SET     & Gerente -- Agente   & set-request      & response \\
		TRAP    & Agente -- Gerente   & trap             & não há   \\
		INFORM  & Qualquer -- Gerente & inform-request   & response \\ 
	\hline
		\end{tabular*}
